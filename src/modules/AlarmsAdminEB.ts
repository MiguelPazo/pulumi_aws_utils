...
import {AlarmsAdminEBConfig, AlarmsAdminEBResult} from "../types";
import {InitConfig} from "../types/module";
import {getInit} from "../config";

class AlarmsAdminEB {
    private static __instance: AlarmsAdminEB;
    ...
    public static getInstance(): AlarmsAdminEB {
        if (this.__instance == null) {
            this.__instance = new AlarmsAdminEB();
        }

        return this.__instance;
    }

    async main(alarmsAdminConfig: AlarmsAdminEBConfig): Promise<AlarmsAdminEBResult> {
        ...
        const result: AlarmsAdminEBResult = {
            logGroup,
            logGroupPolicy,
            eventRules: [],
            eventTargets: []
        ...
        // Create EventBridge rules for Unauthorized Access (AccessDenied/UnauthorizedOperation)
        if (alarmsAdminConfig.monitorUnauthorizedAccess !== false) {
            const unauthorizedAccessRule = this.createUnauthorizedAccessRule(lambdaFunction, logGroup);
            result.eventRules.push(unauthorizedAccessRule.rule);
            result.eventTargets.push(...unauthorizedAccessRule.targets);
        }
    ...
    /**
     * Create EventBridge rule for Unauthorized Access (AccessDenied/UnauthorizedOperation)
     */
    private createUnauthorizedAccessRule(lambdaFunction: aws.lambda.Function, logGroup: aws.cloudwatch.LogGroup): {
        rule: aws.cloudwatch.EventRule;
        targets: aws.cloudwatch.EventTarget[]
    } {
        const rule = new aws.cloudwatch.EventRule(`${this.config.project}-alarms-admin-unauthorized-access`, {
            name: `${this.config.generalPrefix}-alarm-unauthorized-access`,
            description: "Capture CloudTrail events with AccessDenied or UnauthorizedOperation errors",
            eventPattern: JSON.stringify({
                source: ["aws.cloudtrail"],
                "detail-type": ["AWS API Call via CloudTrail"],
                detail: {
            ...
            tags: {
                ...this.config.generalTags,
                Name: `${this.config.generalPrefix}-unauthorized-access`
            }
        });

        const lambdaTarget = new aws.cloudwatch.EventTarget(`${this.config.project}-alarms-admin-unauthorized-lambda-target`, {
            rule: rule.name,
            arn: lambdaFunction.arn,
            inputTransformer: {
                inputPaths: {
                ...
                inputTemplate: `{"type":"alarm-admin","title":"Unauthorized Access Attempt","event":"<eventName>","errorCode":"<errorCode>","user":"<userName>","account":"<accountId>","stack":"${this.config.stack}","region":"<region>","time":"<time>","sourceIp":"<sourceIp>","userAgent":"<userAgent>"}`
            }
        });

        const logTarget = new aws.cloudwatch.EventTarget(`${this.config.project}-alarms-admin-unauthorized-log-target`, {
            rule: rule.name,
            arn: logGroup.arn
        });
